...
public typealias TraqAPIChallengeHandler = (URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?)
...
private var challengeHandlerStore = SynchronizedDictionary<Int, TraqAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: TraqAPIChallengeHandler?
     ...
     */
    open func createURLRequest(urlSession _: URLSessionProtocol, method: HTTPMethod, encoding: ParameterEncoding, headers: [String: String]) throws -> URLRequest {
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = TraqAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in TraqAPI.customHeaders {
            httpHeaders[key] = value
        }
        return httpHeaders
    }
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.Traq.\(UUID().uuidString)")
                }
